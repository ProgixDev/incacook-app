...
//* absent on the detail and seller-dashboard responses.
 String? get sellerName; double? get distanceKm; bool? get inRange; double? get rating; int? get reviewCount;//* Seller pickup coordinates — buyer-feed only, used to place real
...
@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,id,sellerId,name,description,const DeepCollectionEquality().hash(imageUrls),priceCents,originalPriceCents,discountPercent,portionsLeft,const DeepCollectionEquality().hash(cuisineTypes),const DeepCollectionEquality().hash(dishTypes),const DeepCollectionEquality().hash(dietaryTags),const DeepCollectionEquality().hash(allergens),otherAllergens,isAvailable,isVeg,menuCategory,category,fulfillment,prepMinutes,expiresAt,createdAt,updatedAt,const DeepCollectionEquality().hash(extras),sellerName,distanceKm,inRange,rating,reviewCount,lat,lng]);
