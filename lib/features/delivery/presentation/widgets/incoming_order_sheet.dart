...
Future<bool?> showIncomingOrderModal(
  ...
  return showBlurredModalBottomSheet<bool>(
    ...
    isDismissible: false,
    enableDrag: false,
    builder: (_) => IncomingOrderSheet(order: order, canClaim: canClaim),
  );
}
...
class IncomingOrderSheet extends StatefulWidget {
  ...
  /// Gates the "Accepter" button on real delivery constraints (KYC approved).
  /// Stripe payout onboarding is NOT a factor — drivers can accept and earn
  /// before setting up payments (payout setup is enforced at cashout).
  final bool canClaim;
